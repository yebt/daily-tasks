...
<template>
  <div class="max-w-3xl mx-auto p-6 md:p-10">
    <header class="mb-10 flex items-center justify-between">
      ...
      <button @click="showAllDays = !showAllDays"
        class="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold rounded-full border transition-all" :class="showAllDays
        ...
        <template v-for="(day, index) in days" :key="day">
          <section v-if="index === currentDay || showAllDays" class="transition-all duration-200">
            <Transition name="list">
              <button @click="toggleDay(index)" v-if="showAllDays"
                ...
                <em class="i-lucide-chevron-down w-4 h-4 transition-transform text-gray-400"
            ...
            <div v-if="openDay === index" class="mt-2 mb-4 px-4 py-4 bg-white/50 rounded-b-lg space-y-4">
              <div class="flex gap-2">
                <input v-model="newTaskText" @keyup.enter="handleAddTodo(index)" type="text"
              ...
              <div class="space-y-1">
                <TransitionGroup name="vt-fade">
                  <div v-for="todo in sortTodos(todoStore.getTodosByDay(index))" :key="todo.id"
              ...
              <div v-if="todoStore.getTodosByDay(index).length > 0 && showAllDays"
                class="mt-6 pt-4 border-t border-gray-100 flex justify-end">
                <button @click="handleTransfer(index)" :disabled="isTransferring"
