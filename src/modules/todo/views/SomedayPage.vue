...
const somedayTodos = computed(() => {
  return [...todoStore.someDayTodos].sort((a, b) => b.createdAt - a.createdAt)
})

const selectAll = computed({
  get: () => selectedIds.value.size === somedayTodos.value.length && somedayTodos.value.length > 0,
  set: (value) => {
    if (value) {
      selectedIds.value = new Set(somedayTodos.value.map((t) => t.id!))
    } else {
...
<template>
  <div class="max-w-3xl mx-auto p-6 md:p-10">
    ...
    <header class="mb-10">
      <h1 class="text-4xl font-extrabold text-gray-900 tracking-tight">Someday</h1>
      <p class="text-sm text-gray-500 mt-2">{{ somedayTodos.length }} task(s)</p>
    </header>

    <div v-if="somedayTodos.length > 0" class="space-y-4">
      <div
      ...
      >
        ...
        <button
          ...
          :disabled="isTransferring"
          class="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all bg-violet-100 text-violet-700 hover:bg-violet-200 disabled:opacity-50"
        >
          <em class="i-lucide-arrow-right w-4 h-4" />
          Move to Next
        </button>
